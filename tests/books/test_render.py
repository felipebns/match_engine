...
def test_livro_vazio_mostra_so_o_cabecalho(lob):
    linhas = lob.render().splitlines()
    assert len(linhas) == 2
    assert "Ordens de Compra" in linhas[0]


def test_mostra_os_dois_lados(lob, limit_buy, limit_sell):
...
def test_reflete_o_livro_do_requisito_5(lob, limit_buy, limit_sell):
